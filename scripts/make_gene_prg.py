...
class AlignedSeq(object):
    ...
    def get_match_intervals(self):
        ...
        for i in range(self.length):
            ...
            assert (count_match| count_non_match), "Failed to correctly identify match intervals: position %d appeared in both match and non-match intervals" %i
            assert (count_match+count_non_match==1), "Failed to correctly identify match intervals: position %d appeared in %d intervals" %(i,count)
    ...
    def kmeans_cluster_seqs_in_interval(self, interval):#, kmer_size=self.min_match_length):
        ...
        if (interval[1] - interval[0] <= self.min_match_length):
            ...
            logging.debug("interval[1] - interval[0] <= self.min_match_length: %d <= %d", interval[1] - interval[0], self.min_match_length)
            interval_alignment = self.alignment[:, interval[0]:interval[1] + 1]
            interval_seqs = list(remove_duplicates([str(record.seq).replace('-','') for record in interval_alignment]))
            assert len(interval_seqs)==len(list(remove_duplicates(interval_seqs))), "should not have duplicate alternative allele sequences"
            return_id_lists = [[record.id for record in self.alignment if str(record.seq[interval[0]:interval[1] + 1]).replace('-','')==seq] for seq in interval_seqs]
        else:
            ...
            assert len(seq_dict_keys)==len(list(remove_duplicates(seq_dict_keys))), "error, have duplicate dictionary keys"
            assert len([key for key in interval_seq_dict.keys() if key in small_interval_seq_dict.keys()])==0, "error, should have no overlap of keys"
            assert len([key for key in small_interval_seq_dict.keys() if key in interval_seq_dict.keys()])==0, "error, should have no overlap of keys"

            logging.debug("Add classes corresponding to %d small sequences" %len(small_interval_seq_dict.keys()))
            small_return_id_lists = [small_interval_seq_dict[key] for key in small_interval_seq_dict.keys()]

            logging.debug("Now add classes corresponding to %d longer sequences" %len(interval_seq_dict.keys()))
            ...
            if len(interval_seqs) > 1:
                ...
                while(cluster_inertia > 0 and cluster_inertia > pre_cluster_inertia/2 and number_of_clusters <= len(interval_seqs)):
                    number_of_clusters += 1
                    ...
                    if len(not_added) == len(interval_seq_dict[seq]):
                        ...
                        for i in range(len(big_return_id_lists)):
                            if interval_seq_dict[seq][0] in big_return_id_lists[i]:
                                ...
                                return_id_lists.append(big_return_id_lists.pop(i))
                                added_ids.extend(return_id_lists[-1])
                                break
                    else:
                        assert len(not_added) == 0, "Equivalent sequences should be in same part of partition and are not"
                else:
                    logging.warning("Key %s doesn't seem to be in either big keys or small keys")
        assert len(interval_alignment)==sum([len(i) for i in return_id_lists]), "I seem to have lost (or gained?) some sequences in the process of clustering"
        assert len(return_id_lists) > 1, "should have some alternate alleles, not only one sequence, this is a non-match interval"
        ...
        for interval in self.all_intervals:
            if interval in self.match_intervals:
                ...
                if (self.nesting_level == self.max_nesting) or (interval[1] - interval[0] <= self.min_match_length):
                    ...
                    logging.debug("Have reached max nesting level or have a small variant site, so add all variant sequences in interval.")
                    sub_alignment = self.alignment[:, interval[0]:interval[1]+1]
                    logging.debug("Variant seqs found: %s" %list(remove_duplicates([str(record.seq) for record in sub_alignment])))
                    variant_seqs = list(remove_duplicates([str(record.seq).replace('-','') for record in sub_alignment]))
...
def main():
    ...
    if args.verbosity == True:
        logging.basicConfig(filename='%s.log' %prefix,level=logging.DEBUG, format='%(asctime)s %(message)s', datefmt='%d/%m/%Y %I:%M:%S')
        logging.debug("Using debug logging")
    else:
        logging.basicConfig(filename='%s.log' %prefix,level=logging.INFO, format='%(asctime)s %(message)s', datefmt='%d/%m/%Y %I:%M:%S')
    ...
    if os.path.isfile('%s.prg' %prefix):
        ...
        aseq = AlignedSeq(args.MSA, format=args.format, max_nesting=args.max_nesting, min_match_length=args.min_match_length, prg_file=prg_file)
    else:
        aseq = AlignedSeq(args.MSA, format=args.format, max_nesting=args.max_nesting, min_match_length=args.min_match_length)
